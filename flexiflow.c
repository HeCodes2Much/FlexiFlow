...
#define INTERSECTC(x, y, w, h, z) (MAX(0, MIN((x) + (w), (z)->x + (z)->w) - MAX((x), (z)->x)) * MAX(0, MIN((y) + (h), (z)->y + (z)->h) - MAX((y), (z)->y)))
...
void drawbar(Monitor *m)
{
  ...
  for (i = 0; i < LENGTH(tags); i++)
  {
    ...
    drw_text(drw, x + horizpadbar, 0, w, bh_n, wdelta + lrpad / 2, (selmon->alttag ? tagsalt[i] : tags[i]), urg & 1 << i);
    if (ulineall ||
        m->tagset[m->seltags] &
            1 << i) /* if there are conflicts, just move these lines directly
                       underneath both 'drw_setscheme' and 'drw_text' :) */
      drw_rect(drw, x + horizpadbar + ulinepad, bh_n - ulinestroke - ulinevoffset,
...
void drawtab(Monitor *m)
{
  ...
  if (tot_width > mw)
  { // not enough space to display the labels, they need to be truncated
...
void manage(Window w, XWindowAttributes *wa)
{
  ...
  updatewmhints(c);
  {
    ...
    if (XGetWindowProperty(dpy, c->win, netatom[NetClientInfo], 0L, 2L, False, XA_CARDINAL,
                           &atom, &format, &n, &extra, (unsigned char **)&data) == Success &&
        n == 2)
    {
      c->tags = *data;
      for (m = mons; m; m = m->next)
...
void setup(void)
{
  ...
  XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
                  PropModeReplace, (unsigned char *)"flexiflow", 9);
